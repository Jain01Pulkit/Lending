...
library Math {
 ...
 function mulDivRoundingUpHelper(
        uint256 a,
        uint256 b,
        uint256 denominator
    ) internal pure returns (uint256 result) {
        // Wrap inputs into UD60x18
        UD60x18 aUD = ud(a);
        UD60x18 bUD = ud(b);
        UD60x18 denominatorUD = ud(denominator);

        // Perform the multiplication and division
        UD60x18 divisionResult = aUD.mul(bUD).div(denominatorUD);

        // Unwrap the result to uint256
        result = divisionResult.unwrap();

        // Check for remainder and round up if necessary
    ...
    function mulDivRoundingUp(
        ...
        result = mulDivRoundingUpHelper(a,b,denominator);
        if (mulmod(a, b, denominator) > 0) {
